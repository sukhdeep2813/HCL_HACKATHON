...
import { CalendarDays, ClipboardList, Droplet, Moon, Steps } from 'lucide-react'
...
const PatientDashboard = () => {
  ...
  const todaySummary = useMemo(
    () => [
      { label: 'Steps', value: '8,950', icon: <Steps className="h-5 w-5 text-primary" /> },
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          <Card title="Preventive reminders" className="space-y-4">
            {preventiveReminders.map((reminder) => (
              <div key={reminder.id} className="flex items-center justify-between rounded-2xl border border-slate-100 p-4">
                <div>
                  <p className="text-base font-semibold text-slate-900">{reminder.checkupName}</p>
                  <p className="text-sm text-slate-500">Scheduled {new Date(reminder.scheduledDate).toLocaleDateString()}</p>
                </div>
